 ...
 * @Author: liudan6
 -->
<template>
  <div class="sports-page clearfix">
      ...
      <template v-for="(item, index) in newsList" :key="item.docid">
        <div v-if='item.type === 1 || item.type === 2 || item.type === 3 || item.type === 9'
          ...
          <div class="item-imgbox">
            <div class="news-img" :style="{ backgroundImage: 'url(' + item.thumb + ')'}" mode='aspectFill' lazy-load></div>          </div>
        </div>
      </template>
...
<style lang="scss">
  .sports-page {
    ...
    .cmt-num {
      ...
      color: #828282;
      background: url(https://n.sinaimg.cn/sports/imgStore/appSharePage/cmt-bk.svg)
      no-repeat;
    ...
    .video-mark {
      ...
      margin-left: px2rem(7);
      color: #fff;
      background: url(https://n.sinaimg.cn/sports/imgStore/appSharePage/img-sort-bk.svg)
      no-repeat;
      background-size: 100% 100%;
      display: flex;
      justify-content: center;
